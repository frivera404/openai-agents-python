        ...
        while task_payload["attempts"] < self.max_attempts:
            ...
            if not pending:
                ...
                if ok:
                    ...
                    task_payload.setdefault("history", []).append(
                        {"event": "task_done", "msg": msg, "ts": time.time()}
                    )
                    self.state_store.save(task_id, task_payload["status"], task_payload)
                    return task_payload
                ...
                task_payload.setdefault("history", []).append(
                    {"event": "verify_failed", "msg": msg, "ts": time.time()}
                )
            ...
            try:
                ...
                pending["status"] = "done"
                task_payload.setdefault("history", []).append(
                    {"event": "step_done", "step": pending["step_id"], "ts": time.time()}
                )
