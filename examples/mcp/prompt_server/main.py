...
async def get_instructions_from_prompt(mcp_server: MCPServer, prompt_name: str, **kwargs) -> str:
    ...
    logger.info(f"Getting instructions from prompt: {prompt_name}")
    ...
    try:
        ...
        logger.exception("Failed to get instructions: %s", e)
...
"""
    ...
    logger.info(f"Running: {message[:60]}...")
...
async def show_available_prompts(mcp_server: MCPServer):
    ...
    logger.info("User can select from these prompts:")
    for i, prompt in enumerate(prompts_result.prompts, 1):
        logger.info(f"  {i}. {prompt.name} - {prompt.description}")
    logger.info("")
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.info("Starting Simple Prompt Server...")
