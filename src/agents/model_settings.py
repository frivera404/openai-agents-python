    ...
    def to_json_dict(self) -> dict[str, Any]:
        ...
        for field_name, value in dataclass_dict.items():
            if isinstance(value, BaseModel):
                # pydantic v2: model_dump no longer takes mode="json" keyword
                # use model_dump() to get native Python types suitable for JSON.
                json_dict[field_name] = value.model_dump()
            else:
