...
from openai.types.chat.chat_completion_message import (  # type: ignore
    Annotation,
...
class LitellmModel(Model):
    ...
    async def _fetch_response(
        ...
        preserve_thinking_blocks = (
            model_settings.reasoning is not None and model_settings.reasoning.effort is not None  # type: ignore[attr-defined]
        )
        ...
        reasoning_effort = model_settings.reasoning.effort if model_settings.reasoning else None  # type: ignore[attr-defined]
...
class LitellmConverter:
    ...
    def convert_annotations_to_openai(
        ...
        annotations: list[litellm.types.llms.openai.ChatCompletionAnnotation] | None = message.get(  # type: ignore[attr-defined]
            "annotations", None
        )
        if not annotations:
            return None
