                    ...
                    for idx, entry in enumerate(incoming_item.content):
                        ...
                        if entry.type in ("audio", "input_audio"):
                            ...
                            if not entry_transcript:
                                ...
                                if preserved:
                                    entry = entry.model_copy(update={"transcript": preserved})  # type: ignore[attr-defined]

                        new_content.append(entry)

                    if new_content:
                        incoming_item = incoming_item.model_copy(update={"content": new_content})  # type: ignore[attr-defined]
                except Exception:
                ...
                if item.item_id == event.item_id and item.type == "message" and item.role == "user":
                    ...
                    for entry in item.content:
                        if entry.type == "input_audio":
                            copied_entry = entry.model_copy(update={"transcript": event.transcript})  # type: ignore[attr-defined]
                            content.append(copied_entry)
                        else:
                            content.append(entry)  # type: ignore
                    new_history.append(
                        item.model_copy(update={"content": content, "status": "completed"})  # type: ignore[attr-defined]
                    )
                    ...
                    if event.role == "assistant" and existing_item.role == "assistant":
                        ...
                        for idx, ac in enumerate(assistant_incoming):
                            ...
                            if ac.type == "audio":
                                ...
                                if cur_text is not None and ac.text is None:
                                    ...
                                    assistant_new_content.append(ac)
                        updated_assistant = event.model_copy(  # type: ignore[attr-defined]
                            ...
                            if uc.type == "input_audio":
                                ...
                                if transcript is None and idx < len(user_existing_content):
                                    ...
                                    if isinstance(prev, InputAudio) and prev.transcript is not None:
                                        uc = uc.model_copy(update={"transcript": prev.transcript})  # type: ignore[attr-defined]
                                merged.append(uc)
                                ...
                                if (text is None or text == "") and idx < len(
                                    ...
                                    if isinstance(prev, InputText) and prev.text:
                                        uc = uc.model_copy(update={"text": prev.text})  # type: ignore[attr-defined]
                                merged.append(uc)
                            else:
                                merged.append(uc)

                        updated_user = event.model_copy(update={"content": merged})  # type: ignore[attr-defined]
                        ...
                        for idx, sc in enumerate(system_incoming):
                            ...
                            if cur_text is not None and sc.text is None:
                                ...
                                system_new_content.append(sc)
                        updated_system = event.model_copy(update={"content": system_new_content})  # type: ignore[attr-defined]
