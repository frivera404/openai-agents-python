...
try:
    ...
    # Fall back to relative imports when running as a package module.
    from .. import _debug
...
ToolFilterCallable = Callable[["ToolFilterContext", "MCPTool"], Union[bool, Awaitable[bool]]]
    ...
    async def invoke_mcp_tool(
        ...
        if server.use_structured_content and result.structuredContent:
            ...
            if len(result.content) == 1:
                ...
                # For multiple content items, convert each pydantic model to a
                # plain Python structure and serialize to JSON. Using
                # `model_dump()` is compatible across pydantic v2 APIs.
                tool_results = [item.model_dump() for item in result.content]
